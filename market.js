...
(function () {
    ...
    items.forEach(item => {
      ...
      div.innerHTML = `
        ...
        <div style="font-size:11px;margin-top:6px;color:#ccc;">
          ...
          &nbsp;•&nbsp; Время: ${new Date(item.time).toLocaleString()}
        </div>
      `;
      ...
      // Кнопка купить (если не свой лот)
  ...
  async function createMarketLot() {
    ...
    // Проверяем, есть ли у пользователя столько яиц
  ...
  async function buyMarketLot(lotId) {
    ...
    try {
      ...
      // Транзакционно: списать TON у покупателя, добавить TON продавцу (минус комиссия)
